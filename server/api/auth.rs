...
pub async fn auth_admin_middleware(
    ...
    tracing::info!("Auth-Admin middleware: Processing request");
    ...
    if let Some(token) = auth_header {
        tracing::info!("Auth-Admin middleware: Token found, verifying...");
        match state.auth_service.verify_token(token) {
            Ok(claims) => {
                tracing::info!("Auth-Admin middleware: Token verified, getting user from DB");
                // Get user from database
                match state.auth_service.get_user_by_id(&claims.sub).await {
                    Ok(Some(user)) => {
                        tracing::info!("Auth-Admin middleware: User found - ID: {}, Role: {}", user.id, user.role);
                        
                        // Check if user is admin
                        if user.role != "admin" {
                            tracing::error!("Auth-Admin middleware: User role '{}' is not admin", user.role);
                            return Err(StatusCode::FORBIDDEN);
                        }
                        
                        tracing::info!("Auth-Admin middleware: User is admin, proceeding");
                        request.extensions_mut().insert(user);
                        Ok(next.run(request).await)
                    }
                    Ok(None) => {
                        tracing::error!("Auth-Admin middleware: User not found in database");
                        Err(StatusCode::UNAUTHORIZED)
                    }
                    Err(e) => {
                        tracing::error!("Auth-Admin middleware: Database error: {:?}", e);
                        Err(StatusCode::UNAUTHORIZED)
                    }
                }
            }
            Err(e) => {
                tracing::error!("Auth-Admin middleware: Token verification failed: {:?}", e);
                Err(StatusCode::UNAUTHORIZED)
            }
        }
    } else {
        tracing::error!("Auth-Admin middleware: No Authorization header found");
        Err(StatusCode::UNAUTHORIZED)
    }
}

// Middleware to check if user is admin (kept for backward compatibility)
